...
6. **페어 1개 고정** — 토큰 조회는 다중 페어를 반환(실측: 주요 토큰 30개). 유동성 최대 페어를 시작 시점에 고정하고 `tokens.pair_id`에 기록. 페어 전환 시 신규 시계열로 분리(체계 혼합 금지). **[0단계 실측 반영 2026-09-02]** DexScreener 응답은 페어 순서 비정렬(실측: 응답 첫 페어가 3위) → 클라이언트에서 liquidity.usd 내림차순 정렬 후 최대 페어 선택 계약화. 동률 시 tie-break = pairAddress 사전순. 저유동성(<$10k) 페어 선택 시 대시보드에 저신뢰 플래그.
7. **"상위 잔액 집중도"로 명칭 변경** — `getTokenLargestAccounts` 상위 계정은 AMM 풀 볼트·번 주소를 포함(반증 2건 독립 적발). 번 주소·LP 볼트·권한 주소를 필터한 뒤에만 "집중도"로 표기하며, "홀더 수 대리 지표" 표현은 삭제. 전체 홀더 수는 무료 실측 경로 확보 전 보류. **[0단계 실측 반영 2026-09-02]** 필터 없으면 CR20 1.8배 과대(56.32%→31.34% 실증). v1(온체인 태그)은 위양성 — v2 3단 계층 채택: ① 권한 PDA 레지스트리 대조 → ② 풀 계정 파싱(vault 도출) → ③ owner-program 소유 여부.
8. **RPC 호출 주체 = 브라우저 우선** — 데이터센터(Cloudflare) IP→솔라나 공개 RPC는 403/429 상습(반증 실측 429; **0단계 실측 2026-09-02: Worker egress 403 "blocked from this endpoint" 3회 확정**). 집중도 계산은 브라우저에서 수행해 Worker에 POST하거나, Worker egress 실측 통과 시에만 Worker가 직접 호출(→ 실측 결과 불가 확정, 브라우저 경로 확정). 집중도 수집 경로 분기: 소형 민트(홀더 수 적음)=getProgramAccounts 전수 열거(무료 실측 가능), 대형 민트=브라우저 RPC 또는 유료 RPC 확보 시까지 보류.
...
| 단계 | 내용 | 완료 조건 |
|---|---|---|
| 0 (선행 실측 게이트) | ① Worker 크론 CPU 10ms 상한 벤치마크(지표 8종 집계) ② Worker egress→공개 RPC 실측 ③ 페어 선택 규칙 실측(자체 후보 토큰) ④ 번·LP 볼트 주소 특정 및 집중도 필터 실측 | **✅ 2026-09-02 완료** — ① PASS ② 차단 확정(브라우저 경로 유지) ③ PASS ④ CONDITIONAL(필터 v2 채택·경로 분기 반영). 상세: `gates/` — Verifier 검수 전 게이트 Accepted |
